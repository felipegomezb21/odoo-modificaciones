...
class TestStockPickingweight(SavepointCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def _create_product(cls, name, length, width, height):
        ...
        product.onchange_calculate_weight()
        return product

    def _set_product_qty(self, product, qty):
        self.env["stock.quant"]._update_available_quantity(product, self.loc_stock, qty)

    def test_picking_draft_weight(self):
        """
        Data:
            one picking with one move line with 5 units of product
        Test Case:
            get the weight of the picking
        Expected result:
            weight is 5 * 10 * 5 * 3 = 750
            The weight is computed from the expected quantity
        """
        self.assertEqual(self.picking.weight, 750)

    def test_picking_partially_available_weight(self):
        """
        Data:
            one picking with one move line with 5 units of product
        Test Case:
            ...
            get the weight of the picking
        Expected result:
            weight is 1 * 10 * 5 * 3 = 150
            The weight is computed from the available quantity
        """
        self._set_product_qty(self.product, 1)
        self.picking.action_confirm()
        self.picking.action_assign()
        self.picking.invalidate_cache()
        self.assertEqual(self.picking.weight, 150)

    def test_picking_available_weight(self):
        """
        Data:
            one picking with one move line with 5 units of product
        Test Case:
    ...
    def test_picking_done_weight(self):
        """
        Data:
            one picking with one move line with 5 units of product
        Test Case:
        ...
        Expected result:
            weight is 1 * 10 * 5 * 3 = 150
            The weight is computed from the done quantity
        """
        self._set_product_qty(self.product, 1)
        self.picking.action_confirm()
        self.picking.action_assign()
    ...
    def test_picking_cancel_weight(self):
        """
        Data:
            one picking with one move line with 5 units of product
        Test Case:
    ...
    def test_picking_with_canceled_move(self):
        ...
        self.picking.action_assign()
        self.picking.invalidate_cache()
        self.assertEqual(self.picking.weight, 750 * 2)
        self.picking.move_lines[1]._action_cancel()
        self.picking.invalidate_cache()
        self.assertEqual(self.picking.weight, 750)

    def test_product_weight(self):
        self.assertEqual(self.product._get_weight_for_qty(5), 750)
        from_uom = self.env.ref("uom.product_uom_dozen")
        self.assertEqual(
            self.product._get_weight_for_qty(5 * from_uom.factor, from_uom), 750
        )
